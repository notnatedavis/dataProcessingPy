...
#   Decrypts all .txt files in a folder to JPEG images
#   Uses the common module for pixel decoding
...
def decrypt_text_to_image(text_path: str, output_image_path: str) -> None :
    # Convert an encrypted text file to an RGB image
    # The text file contains space‑separated 6‑character pixel strings
...
def main() -> None :
    ...
    if args.dir and args.folder:
        folder_path = os.path.join(args.dir, args.folder)
        if not os.path.isdir(folder_path):
            logging.error(f"Folder not found: {folder_path}")
            return
    else :
        try :
            _, folder_path = common.select_directory_and_folder(purpose="decrypt")
